# Contributing to the Section Menus Shortcode plugin
...
2. **Check if the issue has been fixed** &mdash; if you're not running the latest version of the plugin, please check your code against the repo's `master` branch first (`master` will always contain the latest, stable project code). If you are running the latest version, make sure the problem isn't already resolved in an upcoming [milestone](https://github.com/UCF/Section-Menus-Shortcode/milestones).

### Submit a bug report
If you've followed the steps above and have a valid bug report to submit, you can submit it by [creating a new issue in Github](https://github.com/UCF/Section-Menus-Shortcode/issues/new?template=bug_report.md).
...
We welcome new feature requests from developers across campus.  Please provide as much detail and context as possible to justify the inclusion of your idea in the plugin. We reserve the right to deny feature requests when they don't align with the project's goals, or if said feature is already accomplishable with existing utilities/components.

You can submit a new feature request by [creating a new issue in Github](https://github.com/UCF/Section-Menus-Shortcode/issues/new?template=feature_request.md) and filling out the provided template.
...
2. Clone your fork, and configure the remotes:
   ...
   git clone https://github.com/<your-username>/Section-Menus-Shortcode.git

   # Navigate to the newly cloned directory
   cd Section-Menus-Shortcode

   # Assign the original repo to a remote called "upstream"
   git remote add upstream https://github.com/UCF/Section-Menus-Shortcode.git
   ```
...
5. Commit your changes in logical chunks. Please provide [helpful, readable commit messages](https://chris.beams.io/posts/git-commit/) (avoid nondescriptive messages such as "bugfix" or "minor change").
    ...
    If you're making changes to scss or js files, make sure you're minifying **and committing** those minified file changes.  scss and js file processing should be performed using gulp commands provided in the repo (see [gulpfile.js](https://github.com/UCF/Section-Menus-Shortcode/blob/master/gulpfile.js))
